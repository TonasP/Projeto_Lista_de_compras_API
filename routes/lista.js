...
router.get("/", verificarToken, async (req, res) => {
    ...
    try {
        ...
        WHERE catalogo.usuario_id = $1 AND catalogo.nome ILIKE $2
...
router.post("/", verificarToken, async (req, res) => {
    ...
    try {
        const result = await pool.query(`INSERT INTO public.lista(usuario_id, produto_id, quantidade, status, data_atribuicao)
	    VALUES ($1, $2, $3, $4, $5) returning *;`, [req.user.id, produto_id, quantidade, status, data_atribuicao])
...
router.put("/:id", verificarToken, async(req,res)=>{
    const {produto_id, quantidade, status, data_compra} = req.body
    if (!produto_id || !quantidade || !status || !data_compra ){
        return res.status(400).json({erro: "É necessário preencher todas as informações"})
    }
    try {
        const result = await pool.query(`UPDATE public.lista
	SET produto_id=$1, quantidade=$2, status=$3, data_compra=$4
	WHERE id = $5 and usuario_id = $6 returning *`,[produto_id, quantidade, status, data_compra, req.params.id, req.user.id])
